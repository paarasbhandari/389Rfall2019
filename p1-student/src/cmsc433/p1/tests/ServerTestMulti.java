...
public class ServerTestMulti
{
    ...
    private void testMultiThread(int testNumber, int nrSellers, int nrBuyers)
    {
        ...
        assertFalse("You don't have any recording of how much was spent. Money Spent: "+moneySpent, moneySpent==0);
        //All the bidders are ConservativeBidders, hence the uncollected revenue should be zero.
        assertTrue("Uncollected revenue (" +AuctionServer.getInstance().uncollectedRevenue()+ ") is not zero.",AuctionServer.getInstance().uncollectedRevenue()==0);
