...
from hypothesis.extra.pytestplugin import pytest_runtest_call
        ...
        for mutant in self.mutants:
            ...
            for item in self.collection:
                if not skip:
                    ...
                    #print(reports[1].sections)
                    #print(" " +str(len(reports[1].sections)))
                    if any(("failed" in report.outcome) for report in reports):
                        if "failed" in reports[0].outcome or "failed" in reports[2].outcome:
                            print("Setup/Teardown failed")
                        else:
                            print("\nResult: Failed")
                            print(reports[1].sections[0][1])
